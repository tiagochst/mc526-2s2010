...
%  \node[entity] (emp) [above=of isa] {Empresa} edge (isa);
  ...
  \node[entity] (com) [below left=6.0cm and 5.7cm of usr] {Coment\'ario};
 ...
 % \node[relationship] (upl3) [below =3cm of uni] {Upload} edge  node [pos=0.05, auto, swap] {(0,n)} (uni);
  \node[relationship] (upl) [below =3cm of usr] {Upload} edge  node [pos=0.05, auto, swap] {(0,n)} (usr);
  ...
  \draw[link] (pmu) |- node [pos=0.05, auto, swap] {(1,n)} (pos);
  \draw[link] (doc) |- node [pos=0.05, auto, swap] {(0,n)} (pos);
% UPLOAD  
  \draw[link] (doc.90) -| node [pos=0.55, auto, swap] {(0,n)} (upl);
%  \draw[link] (doc.90) -| node [pos=0.55, auto, swap] {(0,n)} (upl2);
%  \draw[link] (doc.35) |- node [pos=0.55, auto, swap] {(0,n)} (upl1);
%  \draw[link] (doc.35) |- node [pos=0.55, auto, swap] {(0,n)} (upl1);
 

 \draw[link] (usr) -| node [pos=0.25, auto, swap] {(0,n)} (faz);

%%% resposta comentario
  \draw[link] (res.40) |- node [pos=0.25, auto, swap] {(1,1)} (com.-75);
  \draw[link] (res.140) |- node [pos=0.25, auto, swap] {(0,1)} (com.-150);

%%%interesse usuario documento
  \draw[link] (int) -| node [pos=0.25, auto, swap] {(0,1)} (doc.25);
  \draw[link] (int) |- node [pos=0.25, auto, swap] {(0,n)} (usr);

%%% resposta doc
  \draw[link] (resdoc) -| node [pos=0.25, auto, swap] {(0,n)} (doc.-40);
  \draw[link] (resdoc.90) -| node [pos=0.25, auto, swap] {(0,n)} (doc.-30);

%%%ligar documento e documento
  \draw[link] (lig) -| node [pos=0.25, auto, swap] {(0,n)} (doc.-60);
  \draw[link] (lig.90) -| node [pos=0.25, auto, swap] {(0,n)} (doc.-50);

%%%suportar
  \draw[link] (pmu) |- node [pos=0.25, auto, swap] {(0,n)} (sup.90);
  \draw[link] (usr.10) -| node [pos=0.25, auto, swap] {(0,n)} (sup.0);

%%%doc possui comentarios
  \draw[link] (pos2) -| node [pos=0.10, auto, swap] {(0,n)} (doc.120);
  \draw[link] (pos2) -| node [pos=0.10, auto, swap] {(0,n)} (com.0);
...
% Doc
  ...
  \node[attribute] (dti) [below left=0.5cm of doc] {Data inserido} edge (doc);
  \node[attribute] (dtc) [left=of doc] {Data criado} edge (doc);
  \node[attribute] (lo) [below right=1.0cm and 2cm of doc] {L\'{i}ngua utilizador} edge (doc);
  \node[attribute] (lu) [right=of doc] {L\'{i}ngua oficial} edge (doc);
  \node[attribute] (tp) [above left=0cm and 4cm of doc] {Tipo} edge (doc);
...
% usuario
 ...
 \node[attribute] (dti) [above right=1.5cm of usr] {Prefer\^encia} edge (usr);
 \node[attribute] (dti) [above left=0.9cm of usr] {L\'ingua utilizador} edge (usr);
